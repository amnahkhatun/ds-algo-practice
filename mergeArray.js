...
function mergeArraySort(arr1, arr2) {
  while (arr1.length > p1 && arr2.length > p2) {
    if (arr1[p1] <= arr2[p2]) {
      mergeArray.push(arr1[p1]);
      p1++;
    } else {
      mergeArray.push(arr2[p2]);
      p2++;
    }
  }

  if (p2 === arr2.length) {
    var arr1 = arr1.slice(p1);
    mergeArray = mergeArray.concat(arr1);
  } else if (p1 === arr1.length) {
    var arr2 = arr2.slice(p2);
    mergeArray = mergeArray.concat(arr2);
  }
}

// mergeArraySort(arr1, arr2)
...
// let r = firstRecurringCharacter([2, 3, 4, 5]);
// console.log(r);
